...
// role add
$(document).on('submit', '#bt_add', function (ev) {
    $('.error').html('');
    ...
    if (error == false) {
        $.ajax({
            url: base_url + 'business_type/add',
            type: 'post',
            data: formData,
            cache: false,
            contentType: false,
            ...
            success: function (result) {
                if (result.code == 200) {
                        ...
                        setTimeout(function() {
                            window.location.href = base_url + 'business_type';
                        }, 1500);
...
$(document).on('submit', '#bt_edit', function (ev) {
    $('.error').html('');
    ...
    if (error == false) {
        $.ajax({
            url: base_url + 'business_type/update',
            type: 'post',
            data: formData,
            cache: false,
            contentType: false,
