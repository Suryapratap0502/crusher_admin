...
// role add
$(document).on('submit', '#vehicle_type_add', function (ev) {
    $('.error').html('');
    ...
    if (error == false) {
        $.ajax({
            url: base_url + 'vehicle_management/vehicle_type_add',
            type: 'post',
            data: formData,
            cache: false,
            contentType: false,
            ...
            success: function (result) {
                if (result.code == 200) {
                        ...
                        setTimeout(function() {
                            window.location.href = base_url + 'vehicle_management/vehicle_type';
                        }, 1500);
...
$(document).on('submit', '#vehicle_type_edit', function (ev) {
    $('.error').html('');
    ...
    if (error == false) {
        $.ajax({
            url: base_url + 'vehicle_management/vehicle_type_update',
            type: 'post',
            data: formData,
            cache: false,
            contentType: false,
